...
byte mac[] = MAC;
const size_t del_p = 2;  // TODO: A DEFINIT
...
void messageReceived(char* topic, byte* payload, unsigned int length) {
  ...
  if (type == "del") {
    ...
    digitalWrite(del_p, isHigh ? HIGH : LOW);
  } else if (type == "talk") {
...
bool is_comm_ok() {
  ...
  // Sinon, on cherche ce qui ne marche pas.
  ...
  bool time_to_check_connection = (millis() - checkEtherStatusTimer) < checkEtherStatusDelay;
...
void publish(String channel, String payload) {
  ...
  if (mqtt.publish(channel.c_str(), payload.c_str())) {
...
void handle_command() {
  String payload, command = monDecodeur.getCommandString();
  String channel = monDecodeur.getArgString(0);

  if (command == "del") {
    String state = monDecodeur.getArgString(1);
    json.clear();
    json["type"] = "del";
    json["state"] = state == "HIGH" ? true : false;
  } else if (command == "talk") {
    String phrase = "";
    for (size_t i = 1; i < monDecodeur.getArgCount(); i++) {
      phrase += " " + monDecodeur.getArgString(i);
    }
    json.clear();
    json["type"] = "talk";
    json["msg"] = phrase;
